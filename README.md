...
Diagnosing the interconnect stall specifically required Quartus's System Console (Tools → System Debugging Tools → System Console) to interactively read/write memory-mapped components directly over JTAG, since the failure wasn't reproducible from the register-level checks available from Linux alone. The FPGA fabric is programmed via standard JTAG (Quartus Programmer) for each session; no `.rbf`-based boot-time configuration is currently required.
